...
// POST /api/auth - login usando Supabase (tabla: users)
...
export default async function handler(req, res) {
  ...
  let expiresAt = null;
        if (user.created_at && user.expiry_days) {
                  const d = new Date(user.created_at);
                  d.setDate(d.getDate() + user.expiry_days);
                  expiresAt = d.toISOString();
        }

  const searchesLimit = 10;
        const plan = `${user.expiry_days || 30} dias`;
